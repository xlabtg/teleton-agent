...
The Upstash index must be provisioned with the same dimension the embedding provider produces (`local`/`Xenova/all-MiniLM-L6-v2` = 384, `anthropic`/Voyage = 512 or 1024 depending on model). Otherwise every upsert is rejected and the Upstash dashboard shows requests growing with zero vectors. See [Upstash Vector Setup (Step-by-Step)](upstash-vector-setup.md) for a full walk-through and [Semantic Memory](semantic-memory.md) for fallback behavior and manual synchronization details.
...
| `webui.auth_token` | `string` | *auto-generated* | Plaintext Bearer token for API authentication. Legacy/manual configs can set this directly. |
| `webui.auth_token_hash` | `string` | optional | Scrypt hash of the WebUI token generated by the setup wizard. When this is set, startup prints a separate one-time browser exchange link instead of exposing the raw stored token. |
...
Unified external integration registry settings. Individual integrations and encrypted credentials are stored in SQLite and managed through the WebUI or API.
